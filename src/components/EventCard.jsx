...
export default function EventCard({ event, index }) {
  ...
  return (
    <ElectricBorder color="#06b6d4" chaos={0.08} speed={1} borderRadius={12} className={index === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
      <div
      ...
      >
        <article className="event-card-face event-card-front" aria-hidden={isFlipped}>
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-lg bg-[#06b6d4]/10 sm:h-14 sm:w-14">
            {Icon && <Icon className="h-6 w-6 text-[#06b6d4] sm:h-7 sm:w-7" aria-hidden="true" />}
          </div>
          <span className="mt-8 rounded-full border border-[#E2E8F0] bg-white px-3 py-1 text-xs font-medium text-[#06b6d4]">{event.category}</span>
          <h3 className="mt-5 text-3xl font-bold tracking-tight text-[#0F172A] sm:text-4xl">{event.title}</h3>
          <p className="mt-auto pt-8 text-sm font-medium text-[#06b6d4]">Tap to view details</p>
        </article>
        ...
        <article className="event-card-face event-card-back" aria-hidden={!isFlipped}>
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="rounded-full border border-[#E2E8F0] bg-white px-3 py-1 text-xs font-medium text-[#06b6d4]">{event.category}</span>
          </div>
          ...
          <div className="mt-auto pt-6">
            <Link to={event.path} className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[#06b6d4]/30 bg-[#ecfeff] px-5 py-2.5 text-sm font-medium text-[#06b6d4] transition hover:bg-[#06b6d4] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#06b6d4] focus:ring-offset-2 focus:ring-offset-white sm:w-auto">
